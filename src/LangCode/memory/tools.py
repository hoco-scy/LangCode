...
def create_memory_tools(store, manager) -> list:
    """工厂函数：创建记忆工具列表，通过闭包绑定 store 和 manager

    Args:
        store: SQLiteMemoryStore 实例
        manager: MemoryManager 实例
    Returns:
        [memory_save, memory_search, memory_list] 工具列表
    """
    ...
    def memory_search(query: str, top_k: int = 5) -> dict:
        ...
        try:
            records = store.search(query, top_k=top_k)
            if not records:
    ...
    def memory_list(memory_type: Optional[str] = None) -> dict:
        """列出所有长期记忆，可按类型过滤。"""
        try:
            records = store.list_all(memory_type=memory_type)
            results = [
    ...
    return [memory_save, memory_search, memory_list]
